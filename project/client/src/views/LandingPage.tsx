...
export const LandingPage = () => {
  ...
  const handleHeroSearch = (e: React.FormEvent) => {
    e.preventDefault();
    router.push(`/templates${search ? `?q=${encodeURIComponent(search)}` : ''}`);
  };
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3 text-sm text-gray-500">
            {['React', 'Next.js Admin', 'Vue.js SaaS', 'Tailwind Dashboard'].map((tag) => (
              <button
                key={tag}
                type="button"
                onClick={() => router.push(`/templates?q=${encodeURIComponent(tag)}`)}
      ...
      <section className="py-16">
        <div className={marketplaceContainer}>
          <div className="mb-10 flex items-center justify-between">
            ...
            <Link href="/templates" className="flex items-center gap-1 text-sm font-medium text-blue-600 hover:underline">
        ...
        <div className="mx-auto max-w-xl px-4 text-center">
          ...
          <div className="mt-6 flex gap-2">
            <ClientOnly
              fallback={
                <div
                  className="h-12 flex-1 rounded-full border border-gray-200 bg-white"
                  aria-hidden
                />
              }
            >
